...
max <- country_latest_with_data %>% group_by(country) %>% 
  summarise( year = max(year) )

gdp <- distinct( country_latest_with_data[, c('country','gdp_pc_ppp'
                                           ,'gdp_pc_growth','year','population')])
max_year <- merge(x = max, y = gdp, by = c("country","year"), all.x = TRUE)
...
with_counts <- merge(x = max_year, y = data_stats[, c('country', 'c_underw2', 'c_stunt2',
                              'c_wast2','year')],  by=c("country","year")
                     , all.x=TRUE  )
# ==========================================================================================
...
summary_underw <- with_counts%>%dplyr::group_by(country)%>% dplyr::summarize(
                   N_Uw = n() * mean(c_underw2),
                   Percentage_Uw = qwraps2::frmtci(qwraps2::mean_ci(c_underw2) *100 )
                    )
summary_stunt2 <- with_counts%>%dplyr::group_by(country)%>% dplyr::summarize(
                  N_St = n() * mean(c_stunt2),
                  Percentage_St = qwraps2::frmtci(qwraps2::mean_ci( c_stunt2) *100 )
                  )
summary_wast2 <- with_counts%>%dplyr::group_by(country)%>% dplyr::summarize(
                    N_Wt = n() * mean(c_wast2),
                    Percentage_Wt = qwraps2::frmtci(qwraps2::mean_ci( c_wast2) *100)
                )
# ==========================================================================================
# ================ Final table 1                                            =================
# ============================                          =====================================

merge_SW <- merge(x = summary_stunt2, y=summary_wast2 , by = "country", all.x=TRUE)
merge_SWU <- merge(x= merge_SW, y= summary_underw, by="country", all.x=TRUE)

final_table1 <- merge( x=merge_SWU, y= max_year, by="country", all.x=TRUE )
final_table1 <- final_table1[, c(1,8,2:7,9:10)]
colnames(final_table1)[9] <- 'GDP pc'
colnames(final_table1)[10] <- 'Growth (%)'
...
save_html(html = html1, file='table1.html')
...
colnames(Mean_SD_summary ) <- c("Covariate", "Median", "Mean",
                                              ...
                                              "SD", "Coef_Var")  
Mean_SD_summary <- Mean_SD_summary[, c(1,3:8)]
...
save_html(html = html2, file='table2.html')
